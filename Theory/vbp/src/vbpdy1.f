...
C ListOfFiles: vbpdy1 wlepasymk gq11 dist
C===========================================================================
...
c Initial revision
c 
C			      Input parameters are in the common block /STCOM/
C
...
C
     ...
     1 / E866xf/ ixfx
      ...
      EXTERNAL QQ2, QQ3, QQ4A, GQ1, GQ2A, QG1, QG2A, FMSQ
c
      external EwCpl2An
C
      DATA SML / 1.E-6 /
...
C                         Leading Order and 1-loop QQ and GQ terms 
      ...
      DO 10, I = - NFLT, NFLT
          FA0 (I) = VBP_PDF(LSET, NBM, I, XA, AMU,  IRET)
          FB0 (I) = VBP_PDF(LSET, NTG, I, XB, AMU,  IRET)
 10   CONTINUE
...
cjfo
         ...
         ENDIF
C                                                                Term 2
        TMP = ADZINT(QQ2,XA,ONE,AER,RER,ER,IER,1,1)
C	     NN = Intusz(za, qq2a)
C         TMP = GausInt(QQ2,XA,ONE,AER,RER,ER,IER)
         TERM2 = (2./3.) * CPL2 * TMP * FB0(IB)
         SG1QQ = SG1QQ + TERM2
C                                                                Term 3
         TMP = ADZINT(QQ3,XB,ONE,AER,RER,ER,IER,1,1)
         TERM3 = (2./3.) * CPL2 * FA0(IA) * TMP
         SG1QQ = SG1QQ + TERM3
C                                                                Term 4
         TMP = ADZINT(QQ4A,XA,ONE,AER,RER,ER,IER,1,1)
         TERM4 = (4./3.) * CPL2 * TMP
         SG1QQ = SG1QQ + TERM4
      ENDIF
      ENDIF
      ...
      IF (Iord.NE. 1) THEN
        ...
        DO 30 IB = - NFLT, NFLT
          IF (IB.NE.0) Then
             ...
             IF (CPL2 .GT. SML) Then
C                                                              Term 1
                TMP = ADZINT(GQ1,XA,ONE,AER,RER,ER,IER,2,1)
                TERM5 = (1./2.) * CPL2 * TMP * FB0(IB)
cjfo
                SG1GQ = SG1GQ + TERM5
C                                                               Term 2
                TMP = ADZINT(GQ2A,XA,ONE,AER,RER,ER,IER,2,1)
                TERM6  = (1./2.) * CPL2 * TMP
                SG1GQ = SG1GQ + TERM6
             End If
          End If
...
C                                                              Term 1
             TMP = ADZINT(QG1,XB,ONE,AER,RER,ER,IER,2,1)
             TERM7  = .5 * CPL2 * TMP * FA0(IA)
cjfo
             SG1GQ = SG1GQ + TERM7
C                                                                Term 2
             TMP = ADZINT(QG2A,XB,ONE,AER,RER,ER,IER,2,1)
             TERM8  = .5 * CPL2 * TMP
             SG1GQ = SG1GQ + TERM8
          End If
  40    CONTINUE
...
C                                            ...........................Done
      ...
      SIGNLO = SIGLO + ALPI(AMU) * (SG1QQ + SG1GQ)
