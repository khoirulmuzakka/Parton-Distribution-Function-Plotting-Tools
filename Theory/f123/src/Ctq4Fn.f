...
C
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
C
      PARAMETER (MXX = 251, MXQ = 51, MXF = 6)
...
C                                           Now interpolate in x for M1 Q's
      Do 21 Iq = 1, M1
         J1 = J0 + (Nx+1)*(Iq-1) + 1
         Call POLINT2 (XV(Jx), Upd(J1), M1, X, Fq(Iq), Df(Iq))
 21   Continue
C                                          Finish off by interpolating in Q
      Call POLINT2 (QL(Jq), Fq(1), M1, QG, Ftmp, Ddf)
...
C21         Open(IU, File=Tablefile, Status='OLD', Err=101)
